        ...
        return new Promise((resolve, reject) => {
            ...
            pool.getConnection((err, connection) => {
                if (err) {
                    ...
                    logger.debug('Executing query to obtain 1 page of 3 data');
                    if (option == 1) {
                        ...
                        connection.query(designFileDataQuery, [parseInt(userId), '%'+search+'%', parseInt(limit), parseInt(offset), parseInt(userId), '%'+search+'%'], (err, results) => {
                            if (err) {
                                console.log('Error on query on reading data from the file table', err);
                                reject(err);
                            } else {
        ...
        return new Promise((resolve, reject) => {
            ...
            pool.getConnection((err, connection) => {
                if (err) {
                    ...
                    logger.debug('Executing query to obtain 1 page of 3 data');
                    connection.query(designFileDataQuery, [userId, userId,  offset, limit], (err, results) => {
                        if (err) {
                            console.log('Error on query on reading data from the file table', err);
                            reject(err);
                        } else {
